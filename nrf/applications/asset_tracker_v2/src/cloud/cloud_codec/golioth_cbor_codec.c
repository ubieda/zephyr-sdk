			      ...
			      struct cloud_data_cfg *data)
{
	...
	while (!cbor_value_at_end(&map)) {
		err = config_cbor_field_apply(&map, data);
		if (err) {
			return err;
		}
	}
			      ...
			      struct cloud_data_cfg *data)
{
	...
	LOG_HEXDUMP_INF(buffer, writer.enc.bytes_written, "encoded config");
					 ...
					 struct cloud_data_modem_static *data)
{
	...
	LOG_INF("Modem static data TS: %lld", (long long) data->ts);
					 ...
					 struct cloud_data_modem_dynamic *data)
{
	...
	LOG_INF("Modem dynamic data TS: %lld", (long long) data->ts);
			    ...
			    struct cloud_data_battery *bat_buf)
{
	...
	LOG_HEXDUMP_INF(buffer, writer.enc.bytes_written, "encoded data");
			       ...
			       struct cloud_data_ui *ui_buf)
{
	...
	LOG_HEXDUMP_INF(buffer, writer.enc.bytes_written, "encoded ui");
